...
const Reservation = () => {
  ...
  // Vérifier si c'est le 31 décembre (bloqué)
  const isNewYearsEve = useMemo(() => {
    if (!formData.date) return false;
    const date = new Date(formData.date);
    return date.getMonth() === 11 && date.getDate() === 31; // 31 décembre
  }, [formData.date]);
  ...
  const canSubmit = !isNewYearsEve && (!requiresDeposit || depositAcknowledged);

  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <main className="pt-20">
        {/* New Year's Eve WhatsApp Alert */}
        <div className="bg-destructive/20 border-b border-destructive/30">
          <div className="container mx-auto px-4 lg:px-8 py-4">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
              ...
              <span className="text-foreground">
                Réservation <span className="text-destructive font-bold">UNIQUEMENT via WhatsApp</span> (acompte obligatoire)
              </span>
              <a
                href="https://wa.me/33753236352?text=Bonjour%2C%20je%20souhaite%20réserver%20pour%20le%20Réveillon%20du%20Nouvel%20An."
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-[#25D366] hover:bg-[#1da851] text-white font-medium rounded-full transition-colors text-sm"
              >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        value={formData.date}
                        onChange={handleChange}
                        className={`w-full px-4 py-3 bg-background border rounded-sm focus:outline-none transition-colors ${isNewYearsEve ? 'border-destructive' : 'border-border focus:border-accent'}`}
                      />
                      {isNewYearsEve && (
                        <div className="mt-2 p-3 bg-destructive/10 border border-destructive/30 rounded-sm">
                          <p className="text-destructive text-sm font-medium flex items-center gap-2">
                            <AlertTriangle size={16} />
                            Le 31 décembre est réservé uniquement via WhatsApp
                          </p>
                          <a
                            href="https://wa.me/33753236352?text=Bonjour%2C%20je%20souhaite%20réserver%20pour%20le%20Réveillon%20du%20Nouvel%20An."
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 mt-2 px-3 py-2 bg-[#25D366] hover:bg-[#1da851] text-white text-sm font-medium rounded-sm transition-colors"
                          >
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Réserver le 31 via WhatsApp
                          </a>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-3 bg-background border border-border rounded-sm focus:border-accent focus:outline-none transition-colors"
                      >
                        {Array.from({ length: maxGuests - 3 }, (_, i) => i + 4).map((n) => (
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Pour moins de 4 personnes, pas de réservation requise. Venez directement !
                      </p>
                    </div>
                  </div>
