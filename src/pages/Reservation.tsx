...
const Reservation = () => {
  ...
  // Vérifier si c'est le 14 février (Saint-Valentin)
  const isValentinesDay = useMemo(() => {
    if (!formData.date) return false;
    const date = new Date(formData.date);
    return date.getMonth() === 1 && date.getDate() === 14; // Février = mois 1 (0-indexé)
  }, [formData.date]);
  ...
  const canSubmit = (!requiresDeposit || depositAcknowledged) && !isValentinesDay;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 max-w-6xl mx-auto">
              ...
              <div className="bg-card border border-border rounded-lg p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-accent text-sm font-semibold">
                            🌹 Saint-Valentin - Réservation spéciale
                          </p>
                          <p className="text-foreground text-sm mt-1">
                            Pour le 14 février, les réservations se font <strong>exclusivement via WhatsApp</strong>.
                          </p>
                        </div>
                      </div>
                      
                      <a
                        href="https://wa.me/33753236352?text=Bonjour%2C%20je%20souhaite%20r%C3%A9server%20pour%20la%20Saint-Valentin%20(14%20f%C3%A9vrier)"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] hover:bg-[#128C7E] text-white font-medium rounded-sm transition-colors"
                      >
                  ...
                  {requiresDeposit && !isValentinesDay && (
                  ...
                  >
                    {isLoading ? "Envoi en cours..." : isValentinesDay ? "Réservation via WhatsApp uniquement" : requiresDeposit && !depositAcknowledged ? "Paiement requis avant réservation" : "Envoyer ma Demande"}
                  </Button>
                </form>
              </div>
